 ...
 *
 */
if ( ! class_exists( 'MYSF_Field_link_color' ) ) {
  class MYSF_Field_link_color extends MYSF_Fields {
    ...
    public function render() {
      ...
      $color_props = array(
        'color'    => esc_html__( 'Normal', 'mysf' ),
        'hover'    => esc_html__( 'Hover', 'mysf' ),
        'active'   => esc_html__( 'Active', 'mysf' ),
        'visited'  => esc_html__( 'Visited', 'mysf' ),
        'focus'    => esc_html__( 'Focus', 'mysf' )
      );
      ...
      foreach ( $color_props as $color_prop_key => $color_prop_value ) {
        ...
        if ( ! empty( $args[$color_prop_key] ) ) {
          ...
          echo '<div class="mysf--left mysf-field-color">';
          echo '<div class="mysf--title">'. esc_attr( $color_prop_value ) .'</div>';
          echo '<input type="text" name="'. esc_attr( $this->field_name( '['. $color_prop_key .']' ) ) .'" value="'. esc_attr( $value[$color_prop_key] ) .'" class="mysf-color"'. $default_attr . $this->field_attributes() .'/>';
          echo '</div>';
