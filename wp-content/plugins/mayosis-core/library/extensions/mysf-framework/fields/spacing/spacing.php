 ...
 *
 */
if ( ! class_exists( 'MYSF_Field_spacing' ) ) {
  class MYSF_Field_spacing extends MYSF_Fields {
    ...
    public function render() {
      ...
      $is_unit = ( ! empty( $unit ) ) ? ' mysf--is-unit' : '';
      ...
      if ( ! empty( $args['all'] ) ) {
        ...
        echo '<input type="number" name="'. esc_attr( $this->field_name( '[all]' ) ) .'" value="'. esc_attr( $value['all'] ) .'"'. $placeholder .' class="mysf-input-number'. esc_attr( $is_unit ) .'" step="any" />';
        echo ( $unit ) ? '<span class="mysf--label mysf--unit">'. esc_attr( $args['units'][0] ) .'</span>' : '';
        echo '</div>';
        ...
        foreach ( $properties as $property ) {
          ...
          echo '<input type="number" name="'. esc_attr( $this->field_name( '['. $property .']' ) ) .'" value="'. esc_attr( $value[$property] ) .'"'. $placeholder .' class="mysf-input-number'. esc_attr( $is_unit ) .'" step="any" />';
          echo ( $unit ) ? '<span class="mysf--label mysf--unit">'. esc_attr( $args['units'][0] ) .'</span>' : '';
          echo '</div>';
